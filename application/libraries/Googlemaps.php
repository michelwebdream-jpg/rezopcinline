...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyBfDAk5Xb1ZDwMDNj5qBitkVRSec3YlXic'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $center						= "44, 4";		// Sets the default center location (lat/long co-ordinate or address) of the map. If defaulting to the users location set to "auto"
	...
	var $onmousemove				= 'display_coordinate(event);';						// The JavaScript action to perform when the user's mouse moves over the map container
	var $onmouseout					= 'cache_zone_texte_coordonnes();';						// The JavaScript action to perform when the user's mouse exits the map container
	var $onmouseover				= 'affiche_zone_texte_coordonnes();';						// The JavaScript action to perform when the user's mouse enters the map container
	var $onresize					= '';						// The JavaScript action to perform when the maps div changes size
	var $onrightclick				= 'affiche_lien_google_map(event);';						// The JavaScript action to perform when the map is right-clicked
	...
	var $zoom						= 14;						// The default zoom level of the map. If set to "auto" will autozoom/center to fit in all visible markers. If "auto", also overrides the $center parameter
    ...
    //var $ign_key='x23iiydzss21az41qk47a31u'; //DEBUG
    //var $ign_key='sqprcuv3j24volfg8q3ot1yp'; //PROD

    
    //var $ign_key_scan25='evn95zraz85qa5gaur4bqk25';
    
    var $ign_key_scan25='fntnezgc624s1x620jmeqj9c';
    var $ign_key_decouverte='decouverte';
	//var $ign_key_essentiels='essentiels';
	...
	function create_map()
	{
		...
		$this->output_js_contents .= ');
				 ...
				 ';
		$this->output_js_contents .= 'var tiledLayer_carroyages_dfci;
		...
		$this->output_js_contents .= 'var tiledLayer_photos=new google.maps.ImageMapType({
	...
	/*
        $this->output_js_contents .= 'var tiledLayer_photos=new google.maps.ImageMapType({
                        getTileUrl: function(tileCoord,zoom) {
                            var url="https://wxs.ign.fr/'.$this->ign_key_decouverte.'/geoportail/wmts?"+
                                "LAYER=ORTHOIMAGERY.ORTHOPHOTOS"+
                                "&FORMAT=image/jpeg&SERVICE=WMTS&VERSION=1.0.0"+
                                "&REQUEST=GetTile&STYLE=normal&TILEMATRIXSET=PM"+
                                "&TILEMATRIX="+zoom+"&TILEROW="+tileCoord.y+
                ...
                ';
        */
		$this->output_js_contents .= 'var tiledLayer_cartes=new google.maps.ImageMapType({
			getTileUrl: function(tileCoord,zoom) {
	...
	$this->output_js_contents .= 'var tiledLayer_cartes=new google.maps.ImageMapType({
		getTileUrl: function(tileCoord,zoom) {
			var url="https://wxs.ign.fr/'.$this->ign_key_scan25.'/geoportail/wmts?"+
				"LAYER=GEOGRAPHICALGRIDSYSTEMS.MAPS"+
				"&FORMAT=image/jpeg&SERVICE=WMTS&VERSION=1.0.0"+
				"&REQUEST=GetTile&STYLE=normal&TILEMATRIXSET=PM"+
				"&TILEMATRIX="+zoom+"&TILEROW="+tileCoord.y+
...
';*/
	/*
        $this->output_js_contents .= 'var tiledLayer_plan=new google.maps.ImageMapType({
                        getTileUrl: function(tileCoord,zoom) {
                            var url="https://wxs.ign.fr/'.$this-> ign_key_decouverte.'/geoportail/wmts?"+
        ...
        $this->output_js_contents .= 'var tiledLayer_altitude=new google.maps.ImageMapType({
                        getTileUrl: function(tileCoord,zoom) {
                            var url="https://data.geopf.fr/wmts?"+
				...
				$this->output_js_contents .= 'var tiledLayer_altitude=new google.maps.ImageMapType({
					getTileUrl: function(tileCoord,zoom) {
						var url="https://wxs.ign.fr/altimetrie/geoportail/wmts?"+
							"LAYER=ELEVATION.SLOPES"+
							"&FORMAT=image/jpeg&SERVICE=WMTS&VERSION=1.0.0"+
							"&REQUEST=GetTile&STYLE=normal&TILEMATRIXSET=PM"+
							"&TILEMATRIX="+zoom+"&TILEROW="+tileCoord.y+
        ...
        $this->output_js_contents .= 'var tiledLayer_scan25tour=new google.maps.ImageMapType({
                        getTileUrl: function(tileCoord,zoom) {
                            var url="https://data.geopf.fr/private/wmts?"+
		...
		/*	$this->output_js_contents .= 'tiledLayer_carroyages_dfci=new google.maps.ImageMapType({
				getTileUrl: function(tileCoord,zoom) {
					var url="https://wxs.ign.fr/environnement/geoportail/wmts?"+
						"LAYER=GEOGRAPHICALGRIDSYSTEM.DFCI"+
						"&FORMAT=image/png&SERVICE=WMTS&VERSION=1.0.0"+
						"&REQUEST=GetTile&STYLE=normal&TILEMATRIXSET=PM"+
						"&TILEMATRIX="+zoom+"&TILEROW="+tileCoord.y+
		...
		$this->output_js_contents .= 'var tiledLayer_openstreetmap=new google.maps.ImageMapType({
			getTileUrl: function(tileCoord,zoom) {
				// OpenStreetMap utilise le système de tuiles standard XYZ (Slippy Map)
				// Google Maps ImageMapType passe tileCoord.x et tileCoord.y
				// IMPORTANT: Google Maps ImageMapType utilise déjà le format OSM standard!
				// Les coordonnées sont déjà dans le bon format, pas besoin de conversion Y
				var numTiles = 1 << zoom; // 2^zoom
				
				// Coordonnée X : gérer le wrap-around
				var x = tileCoord.x;
				x = ((x % numTiles) + numTiles) % numTiles;
				
				// Coordonnée Y : utiliser directement (Google Maps ImageMapType = format OSM)
				var y = tileCoord.y;
				
				// Limites de sécurité
				if (y < 0) y = 0;
				if (y >= numTiles) y = numTiles - 1;
        ...
        // Centre par défaut sur la France (utilisé par toutes les cartes : Google Maps, IGN et OpenStreetMap)
        $this->output_js_contents .= 'var myLatlng = new google.maps.LatLng(46,2.5);';
        
        
		$this->output_js_contents .= '
				var myOptions = {
			  		';
		if ($this->zoom=="auto") { $this->output_js_contents .= 'zoom: 14,'; }else{ $this->output_js_contents .= 'zoom: 6,'; }
		// Toujours définir le centre sur la France (46, 2.5) - sera remplacé par géolocalisation si center="auto"
		$this->output_js_contents .= '
					center: myLatlng,';
		...
		if (count($this->map_types_available)) {
            ...
            array_push($map_types, '"IGNLayer_photos"');
            array_push($map_types, '"IGNLayer_cartes"');
            array_push($map_types, '"IGNLayer_plan"');
            array_push($map_types, '"IGNLayer_altitude"');
            array_push($map_types, '"IGNLayer_scan25tour"');
            array_push($map_types, '"OSMLayer"');
            //array_push($map_types, '"IGNLayer_carroyages_dfci"');
		}
        ...
        controlUI.title = "Active le suivi des utilisateurs";
        ...
        controlImage.innerHTML = "<img alt=\"trace\" src=\"images/trace.png\" width=\"20\" height=\"20\" style=\"margin-top:7px\">";
        controlImage.style.display = "inline";
        ...
        controlText.innerHTML = "Activer le suivi <a href=\"https://www.web-dream.fr/app/aide-pour-le-suivi-equipe-dans-rezo-pc-inline/\" target=\"_blank\" style=\"vertical-align: top;margin-left:5px;\">?</a>";
        ...
        controlUI.addEventListener("click", function() {
            if (trace_polyline_user) {
                trace_polyline_user=false;
                controlText.innerHTML = "Activer le suivi <a href=\"https://www.web-dream.fr/app/aide-pour-le-suivi-equipe-dans-rezo-pc-inline/\" target=\"_blank\" style=\"vertical-align: top;margin-left:5px;\">?</a>";
                effface_polyline_user();
                controlImage.innerHTML = "<img alt=\"trace\" src=\"images/trace.png\" width=\"20\" height=\"20\" style=\"margin-top:7px\">";
            }else{
                trace_polyline_user=true;
                controlText.innerHTML = "Désactiver le suivi <a href=\"https://www.web-dream.fr/app/aide-pour-le-suivi-equipe-dans-rezo-pc-inline/\" target=\"_blank\" style=\"vertical-align: top;margin-left:5px;\">?</a>";
                affiche_polyline_user();
                controlImage.innerHTML = "<img alt=\"trace\" src=\"images/trace_vert.png\" width=\"20\" height=\"20\" style=\"margin-top:7px\">";
            }
    ...
    ';
        ...
        controlUI.style.textAlign = "center";
        controlUI.title = "Active Carroyages DFCI";
        controlDiv.appendChild(controlUI);
        ...
        controlText.style.paddingRight = "5px";
        controlText.style.paddingLeft = "5px";
        controlText.innerHTML = "Carroyage DFCI";
        controlText.style.display = "inline";
        controlText.style.verticalAlign = "top";
        controlUI.appendChild(controlText);
    ...
    ';
        ...
        $this->output_html .= '<div id="'.$this->map_div_id.'" style="width:'.$this->map_width.'; flex:1;"'.(($this->class != "") ? ' class="'.$this->class.'"' : '').'></div>';
