...
<h1  style=" overflow: hidden; float: left;margin-top: 5px;"><span id="titre_page_modification_activite"></span></h1>
...
<details close>
          ...
          <p id="summary_edition_activite" style="text-align:center;font-size:14px;">Vous pouvez modifier le nom de cette activité, le responsable PC ainsi que l'adresse. Vous pouvez aussi ajouter des moyens ou en enlever.<br />Pour enregistrer les modifications cliquez sur le bouton "sauvegarder" en bas de page.</p>
        <p id="summary_nouvelle_activite" style="text-align:center;font-size:14px;">Vous devez renseigner le nom de cette activité et éventuellement le nom du responsable et l'adresse. Vous devez aussi ajouter des moyens.<br />Pour enregistrer la nouvelle activité, cliquez sur le bouton "sauvegarder" en bas de page.</p>
        </details>
<hr />
<div>
    <h2>Informations générales sur l'activité / DPS</h2>
    <div class="formulaire_saisie_activite">
        
        <p>
        <label for="input_nom_activite">Nom de l'activité * : </label>
        <input type="text" name="nom_activite" id="input_nom_activite" value="" style="display:inline-block;margin-left: 10px;width:100%"  maxlength="256"/><br />
        </p>
        <p>
        <label for="input_nom_responsable_activite">Nom du responsable PC : </label>
        <input type="text" name="nom_responsable_activite" id="input_nom_responsable_activite" value="" style="display:inline-block;margin-left: 10px;width:100%"   maxlength="70" /><br />
        </p>
        
        <p>
        <label for="input_adresse_activite">Lieu : </label>
        <textarea name="adresse_activite" id="input_adresse_activite" value="" cols="50" rows="4" maxlength="256" style="display:inline-block;margin-left: 10px;width:100%"/>
        </p>
            
        <p>
            <label for="input_nature_activite">Nature du dispositif : </label>
        <textarea name="nature_activite" id="input_nature_activite" value="" cols="50" rows="5" maxlength="256" style="display:inline-block;margin-left: 10px;width:100%"/>
        </p>
        
        <p>
            <label for="input_remarque_activite">Consignes particulières initiales<br>Remarques<br>Informations diverses : </label>
        <textarea name="remarque_activite" id="input_remarque_activite" value="" cols="50" rows="5" maxlength="256" style="display:inline-block;margin-left: 10px;width:100%"/>
        </p>
               
    </div>
    <hr />
    
    <h2>Moyens engagés sur le terrain</h2>
...
function affiche_detail_activite_edition(liste_des_codes){
    ...
    .done(function(data, textStatus, jqXHR ) {
        ...
        else
        {
                ...
                var page_en_cours=table_detail_activite_edition.page();
                var page_max=table_detail_activite_edition.page.info().pages;
                ...
                if (data_table_list_des_detail_activites_edition.length>0){
                    ...
                    if (page_en_cours<=page_max){
                        table_detail_activite_edition.page(page_en_cours).draw('page');
                    }else{
                        table_detail_activite_edition.page(page_max).draw('page');
                    }
                }else{
...
function enregistre_activite(){
    if ($('#input_nom_activite').val()==="")
	{
        ...
        var nom_responsable_activite=$('#input_nom_responsable_activite').val();
        nom_responsable_activite = nom_responsable_activite.replace(/[&<>{}]/g, "-");
        
        var adresse_activite=$('#input_adresse_activite').val();
        adresse_activite = adresse_activite.replace(/[&<>{}]/g, "-");
        
        var nature_activite=$('#input_nature_activite').val();
        nature_activite = nature_activite.replace(/[&<>{}]/g, "-");
        
        var remarque_activite=$('#input_remarque_activite').val();
        remarque_activite = remarque_activite.replace(/[&<>{}]/g, "-");
		...
		{
			enregistre_modification_activite_sur_serveur(Id_editing_activite_edition,nom_nouvelle_activite,nom_responsable_activite,adresse_activite,nature_activite,remarque_activite,listofcodeasauver,temp1.length-1);
		}else{
			sauve_nouvelle_activite_sur_serveur(nom_nouvelle_activite,nom_responsable_activite,adresse_activite,nature_activite,remarque_activite,listofcodeasauver,temp1.length-1);
		}
...
function sauve_nouvelle_activite_sur_serveur(nom_de_lactivite,nom_responsable_activite,adresse_activite,nature_activite,remarque_activite,listofcodeasauver,Qte){
    
    var $data={
            mon_code:Global.code_administrateur,
            nom_de_lactivite:nom_de_lactivite,
            nom_responsable:nom_responsable_activite,
            adresse:adresse_activite,
            nature_activite:nature_activite,
            remarque:remarque_activite,
...
function enregistre_modification_activite_sur_serveur(Id, nom_de_lactivite,nom_responsable_activite,adresse_activite,nature_activite,remarque_activite,listofcodeasauver,Qte){
...
function ajoute_utilisateur_table_contact_PC(codeapasser){
  ...
  ;
    ...
    if (data_pass!==""){
        ...
        $('#input_nom_responsable_activite').val(data_pass.nom_responsable);
        $('#input_adresse_activite').val(data_pass.adresse);
        $('#input_nature_activite').val(data_pass.nature_activite);
        $('#input_remarque_activite').val(data_pass.remarque);
