...
function create_contenu_infowindow(code,indicatif,latlng,etat,iconId,nom,prenom,tel,precision,distance){
    ...
    /*var Contenu_info='Indicatif : '+indicatif+'<br />Nom : '+nom+'<br />Prénom : '+prenom+'<br />Tel : '+tel+'<br />Lat : '+latlng.lat()+'<br />Long : '+latlng.lng()+'<br />Précision : '+precision+' m'+'<br />Distance : '+distance+' Km'+'<br /><br /><center><strong>Statut : ' + etat_texte + '</strong><button type="button" class="bouton_infowindow" onclick="change_statut(\''+code+'\')">Modifier le statut</button><button type="button" class="bouton_infowindow" onclick="change_iconid(\''+code+'\')">Modifier l\'icône</button><button type="button" class="bouton_infowindow" onclick="modifier_indicatif_utilisateur(\''+code+'\')">Modifier l\'indicatif</button></center>';*/
...
function zoomtofit(force){
	...
	if (centrage_sur_utilisateur || force){
        ...
        for (var i = 0; i < markers_fixe_on_map.length; i++) {
            bounds.extend(markers_fixe_on_map[i].getPosition());
        }
        for (var i = 0; i < marker_ma_position_on_map.length; i++) {
            bounds.extend(marker_ma_position_on_map[i].getPosition());
        }
