...
if ($is_local) {
	file_put_contents($log_file, date('Y-m-d H:i:s') . " - [envoi_message_user] Mode LOCAL détecté - Proxy vers serveur PRODUCTION\n", FILE_APPEND);
	...
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, 'https://www.web-dream.fr/dev/rezo_flash_code/envoi_message_user.php');
	...
	if (curl_errno($ch)) {
		$error = curl_error($ch);
		file_put_contents($log_file, date('Y-m-d H:i:s') . " - [envoi_message_user] ERREUR cURL: " . $error . "\n", FILE_APPEND);
		curl_close($ch);
		ob_clean();
		echo "-1";
		exit;
	...
	if ($httpCode >= 400) {
		file_put_contents($log_file, date('Y-m-d H:i:s') . " - [envoi_message_user] ERREUR HTTP: " . $httpCode . "\n", FILE_APPEND);
		ob_clean();
		echo "-1";
		exit;
	}
	
	file_put_contents($log_file, date('Y-m-d H:i:s') . " - [envoi_message_user] Réponse du serveur production reçue (HTTP $httpCode)\n", FILE_APPEND);
	ob_clean();
	echo $output;
	exit;
}
